...
app.layout = html.Div([
    ...
    html.Div([
        html.Button("Download CBMP gemiddelden als CSV", id="btn-download-cbmp-csv", n_clicks=0),
        dcc.Download(id="download-cbmp-csv")
...
@app.callback(
    Output("download-cbmp-csv", "data"),
    Input("date-picker", "date"),
    Input("line1", "value"),
    Input("line2", "value"),
    Input("btn-download-cbmp-csv", "n_clicks"),
    prevent_initial_call=True
)
def generate_cbmp_csv(date_str, line1, line2, n_clicks):
    cbmp_df = process_cbmp(date_str)

    if cbmp_df.empty:
        return dcc.send_data_frame(pd.DataFrame().to_csv, filename="cbmp_result.csv")

    df = cbmp_df.copy()
    df['Kwartier'] = df['Datetime'].dt.floor('15min')

    results = df.groupby('Kwartier').apply(lambda g: pd.Series({
        'Gemiddelde ELIA_POS boven hulplijn': g[g['ELIA_POS'] > line1]['ELIA_POS'].mean(),
        'Gemiddelde ELIA_NEG onder hulplijn': g[g['ELIA_NEG'] < line2]['ELIA_NEG'].mean()
    })).reset_index()

    results['Datum'] = pd.to_datetime(date_str).date()

    return dcc.send_data_frame(results.to_csv, filename="cbmp_result.csv", index=False)
